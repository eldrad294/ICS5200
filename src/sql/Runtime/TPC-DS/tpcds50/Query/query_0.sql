...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between to_char(to_date('2001-02-20','yyyy/mm/dd'),'yyyy-mm-dd')
				and (to_char(to_date('2001-02-20','yyyy/mm/dd') + 30,'yyyy-mm-dd'))
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
(select avg(quantity*list_price) average_sales
  from (select ss_quantity quantity
       ...
       where ws_sold_date_sk = d_date_sk
         and d_year between 2000 and 2000 + 2) x)
 select * from ( select  this_year.* from
...
from
  ...
  ,call_center
where
    d_date between to_char(to_date('2000-5-01','yyyy/mm/dd'),'yyyy-mm-dd') and
           (to_char(to_date('2000-5-01','yyyy/mm/dd') + 60,'yyyy-mm-dd'))
...
select * from (select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (to_char(to_date(d_date,'yyyy/mm/dd'),'yyyy-mm-dd') < to_char(to_date('2002-06-12','yyyy/mm/dd'),'yyyy-mm-dd'))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (to_char(to_date('2002-06-12','yyyy/mm/dd') - 30,'yyyy-mm-dd'))
                    and (to_char(to_date('2002-06-12','yyyy/mm/dd') + 30,'yyyy-mm-dd'))
...
and d_date between to_char(to_date('2002-01-27','yyyy/mm/dd'),'yyyy-mm-dd') and
        (to_char(to_date('2002-01-27','yyyy/mm/dd') + 90,'yyyy-mm-dd'))
...
select * from (select  i_item_id
 ...
 and d_date between to_char(to_date('2002-04-13','yyyy/mm/dd'),'yyyy-mm-dd') and (to_char(to_date('2002-04-13','yyyy/mm/dd') +  60,'yyyy-mm-dd'))
...
select * from (select  
   w_state
  ,i_item_id
  ,sum(case when (to_char(to_date(d_date,'yyyy/mm/dd'),'yyyy-mm-dd') < to_char(to_date('2001-04-26','yyyy/mm/dd'),'yyyy-mm-dd'))
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (to_char(to_date('2001-04-26','yyyy/mm/dd') - 30,'yyyy-mm-dd'))
                and (to_char(to_date('2001-04-26','yyyy/mm/dd') + 30,'yyyy-mm-dd'))
 group by
    w_state,i_item_id
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('ivory','salmon','almond'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2002
 and     d_moy                   = 2
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('blanched','light','khaki','ghost','olive','hot') and
...
select * from (select   
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 2002
 	and t_time between 41472 and 41472+28800 
...
with ss as
 ...
 from store_sales,
      ...
      store
 where ss_sold_date_sk = d_date_sk
       and d_date between to_char(to_date('1999-08-07','yyyy/mm/dd'),'yyyy-mm-dd')
                  and (to_char(to_date('1999-08-07','yyyy/mm/dd') +  30,'yyyy-mm-dd'))
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
with ssr as
 (select  s_store_id as store_id,
  ...
  from store_sales left outer join store_returns on
     ...
     promotion
 where ss_sold_date_sk = d_date_sk
       and d_date between to_char(to_date('2000-08-12','yyyy/mm/dd'),'yyyy-mm-dd')
                  and (to_char(to_date('2000-08-12','yyyy/mm/dd') +  30,'yyyy-mm-dd'))
       and ss_store_sk = s_store_sk
       and ss_item_sk = i_item_sk
...
select * from (select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between to_char(to_date('1998-06-17','yyyy/mm/dd'),'yyyy-mm-dd') and (to_char(to_date('1998-06-17','yyyy/mm/dd') +  60,'yyyy-mm-dd'))
 and i_manufact_id in (838,330,50,870)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1998-07-10','1998-10-14','1998-11-03')))
...
select * from (select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Stringtown'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  19705
   and ib_upper_bound   <=  19705 + 50000
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (2002) and
        ((i_category in ('Books','Men','Music') and
          i_class in ('sports','sports-apparel','classical')
         )
      or (i_category in ('Children','Electronics','Home') and
          i_class in ('infants','televisions','tables') 
        ))
...
i_manufact_id = 651
and i_item_sk = ws_item_sk 
and d_date between to_char(to_date('2002-01-06','yyyy/mm/dd'),'yyyy-mm-dd') and
        (to_char(to_date('2002-01-06','yyyy/mm/dd')+ 90,'yyyy-mm-dd'))
...
from
  ...
  ,web_site
where
    d_date between to_char(to_date('2000-3-01','yyyy/mm/dd'),'yyyy-mm-dd') and
           (to_char(to_date('2000-3-01','yyyy/mm/dd') + 60,'yyyy-mm-dd'))
...
where
    d_date between '2002-5-01' and
           (to_char(to_date('2002-5-01','yyyy/mm/dd') + 60,'yyyy-mm-dd'))
and ws1.ws_ship_date_sk = d_date_sk
and ws1.ws_ship_addr_sk = ca_address_sk
and ca_state = 'VA'
and ws1.ws_web_site_sk = web_site_sk
...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between to_char(to_date('1998-01-14','yyyy/mm/dd'),'yyyy-mm-dd')
				and (to_char(to_date('1998-01-14','yyyy/mm/dd') + 30,'yyyy-mm-dd'))
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
