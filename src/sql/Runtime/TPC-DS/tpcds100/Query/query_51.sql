WITH web_v1
     AS (SELECT ws_item_sk
                item_sk,
                d_date,
                SUM(SUM(ws_sales_price))
                  over (
                    PARTITION BY ws_item_sk
                    ORDER BY d_date ROWS BETWEEN unbounded preceding AND CURRENT
                  ROW)
                   cume_sales
         FROM   web_sales,
                date_dim
        ...
        FROM   (SELECT item_sk,
                       ...
                       Max(web_sales)
                         over (
                           ...
                           ORDER BY d_date ROWS BETWEEN unbounded preceding AND
                         CURRENT
                         ROW)
                       web_cumulative,
                       Max(store_sales)
