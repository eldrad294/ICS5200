...
where 
	...
	and d_date between to_char(to_date('1998-04-06','yyyy/mm/dd'),'yyyy-mm-dd')
				and (to_char(to_date('1998-04-06','yyyy/mm/dd') + 30,'yyyy-mm-dd'))
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select * from (select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (to_char(to_date(d_date,'yyyy/mm/dd'),'yyyy-mm-dd') < to_char(to_date('2002-02-27','yyyy/mm/dd'),'yyyy-mm-dd'))
...
select * from (select  i_item_id
 ...
 and d_date between to_char(to_date('1999-02-21','yyyy/mm/dd'),'yyyy-mm-dd') and (to_char(to_date('1999-02-21','yyyy/mm/dd') +  60,'yyyy-mm-dd'))
...
select * from (select  
   w_state
  ,i_item_id
  ,sum(case when (to_char(to_date(d_date,'yyyy/mm/dd'),'yyyy-mm-dd') < to_char(to_date('2001-04-02','yyyy/mm/dd'),'yyyy-mm-dd'))
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('blush','slate','cream','green','frosted','deep') and
...
with ss as
 ...
 from store_sales,
      ...
      store
 where ss_sold_date_sk = d_date_sk
       and d_date between to_char(to_date('2000-08-23','yyyy/mm/dd'),'yyyy-mm-dd')
                  and (to_char(to_date('2000-08-23','yyyy/mm/dd') +  30,'yyyy-mm-dd'))
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('2002-06-19','2002-08-07','2002-11-02')))
...
select * from (select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Walnut Grove'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  44090
   and ib_upper_bound   <=  44090 + 50000
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (1998) and
        ((i_category in ('Music','Sports','Women') and
          i_class in ('pop','athletic shoes','dresses')
         )
      or (i_category in ('Children','Men','Jewelry') and
          i_class in ('toddlers','shirts','pendants') 
        ))
